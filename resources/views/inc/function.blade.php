...
@php
        ...
        if (!function_exists('setPageTitle')) :
            function setPageTitle($page_name) {
                echo $page_name . ' | e-services | Gestion des AMC et AMM';
            }
        endif;

        if (!function_exists('set_breadcrumb_menu')) :
            function set_breadcrumb_menu($sousmenu, $menu) {

                $menu_ = ucfirst($menu);
                $sousmenu_ = ucfirst($sousmenu);

                echo '<li class="breadcrumb-item"><a href="javascript:void(0);">'. $menu_ .'</a></li>
                        <li class="breadcrumb-item active" aria-current="page"><span>' . $sousmenu_ .'</span></li>';
